...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  // Рассчитываем квадрат расстояния по теореме Пифагора
...
function getParallelepipedDiagonal(a, b, c) {
  const parallelepipedDiagonal = Math.sqrt(a * a + b * b + c * c);
...
function isSafeInteger(number) {
  const safedInteger = Number.isSafeInteger(number);
  return safedInteger;
}
